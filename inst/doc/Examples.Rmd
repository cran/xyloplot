...
`xyloplot` also accepts an `rhs` argument, causing the xylophones to split in the middle, and plotting left hand side distributions `x` and right hand side ones from `rhs`.
...
Note that graphical options, e.g. `col`, are recycled over the distributions going from left to right, regardless of whether they are lhs or rhs.
